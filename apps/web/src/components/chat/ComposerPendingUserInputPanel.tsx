...
const ComposerPendingUserInputCard = memo(function ComposerPendingUserInputCard({
  ...
  // Every new question shows in full; folding is a per-question convenience.
  useEffect(() => {
    setIsFolded(false);
  }, [activeQuestion?.id]);
  ...
  useEffect(() => {
    if (!activeQuestion || isResponding || isFolded) return;
  ...
  }, [activeQuestion, isResponding, isFolded]);
  ...
  return (
    <div className="px-4 py-3 sm:px-5">
      <button
        ...
        className={cn(
          "-mx-1 flex w-[calc(100%+0.5rem)] items-center gap-3 rounded-md px-1 py-0.5 text-left outline-none transition-colors hover:bg-muted/30 focus-visible:ring-1 focus-visible:ring-primary/25",
          isFolded ? "mb-0" : "mb-2",
        )}
      >
        ...
        <ChevronDownIcon
          className={cn(
            "ml-auto size-4 shrink-0 text-muted-foreground/60 transition-transform duration-150",
